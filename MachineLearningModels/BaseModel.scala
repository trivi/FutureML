...
//val SelectedLabel = ColumnsLabel(1)
for ( SelectedLabel <- ColumnsLabel ) {
  ...
  val pipeline = ( new Pipeline()
    .setStages(arrayIndexers ++ arrayEncoders ++ Array(assembler, lr)) )
  ...
  n_assign.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").save("n_groups_LR")
  /***********************************************************************************************************/
