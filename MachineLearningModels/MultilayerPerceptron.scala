...
//val ColumnsNumerical = Array("Priority Weight", "Urgency", "Impact", "Priority", "Assigned Group")

// Seleccionamos la columna label (cambiar por for después)
...
//for ( SelectedLabel <- ColumnsLabel ) {
  ...
  var logregdata = logregdataall.withColumnRenamed(SelectedLabel, "label").na.drop()
  ...
  //Convertimos a double las columnas numéricas
  logregdata = logregdata.withColumn("Priority Weight", logregdata("Priority Weight").cast(DoubleType))
  ...
  for ( i <- ColumnsCategorical.diff(Array("label")) ) {
    ColumnsFeatures ++= Array( "indexed" ++ i.capitalize )
  }
  ...
  //Set layers (first layer is the input layer and the last is the output layer)
  val layers = Array[Int]( ColumnsFeatures.length, 50, logregdata.groupBy("label").count().count.toInt )

  // Elección del modelo
  val mp = new MultilayerPerceptronClassifier().setLayers(layers).setLabelCol("indexedLabel")


  // Chain indexers and tree in a Pipeline.
  val pipeline = ( new Pipeline()
    .setStages(arrayIndexers ++ Array(assembler, mp)) )
  ...
  n_assign.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").save("n_groups_MP")
  */
  /***********************************************************************************************************/
