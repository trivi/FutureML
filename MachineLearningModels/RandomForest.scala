...
for ( SelectedLabel <- ColumnsLabel ) {
  ...
  // Elección del modelo
  val rf = new RandomForestClassifier().setLabelCol("indexedLabel").setNumTrees(5)

  // Chain indexers and tree in a Pipeline.
  val pipeline = ( new Pipeline()
    .setStages(arrayIndexers ++ arrayEncoders ++ Array(assembler, rf)) )
  ...
  n_assign.coalesce(1).write.format("com.databricks.spark.csv").option("header","true").save("n_groups_RF")
  /***********************************************************************************************************/
